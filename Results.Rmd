...
LMMs enforces constraints on the distributions of effect sizes. To test whether those constraints influenced the cross-atlas comparisons, we can fit simple linear regression to see if we find similar variance explained.
...
for(i in unique(all$meas)){
  ...
  for(j in unique(all$pheno)){
      ...
      vertex_lm = round(lm_indiv$LM_indiv[which(lm_indiv$meas == i & lm_indiv$pheno == j & lm_indiv$atlas == "vertices")], digits = 2)
...
It seems as though the LM and LMM estimate diverge more when considering coarse atlases (compared with fine-grained ones) which suggests that the assumptions enforced by the LMM result in an overestimation of variance explained. The more high-dimensional the atlases are, the more similar the LMM and LM estimates are to one another and the more similar both LM and LMM estimates are compared with the vertex-wise estimates. However, there is a point where LMMs give estimates close to the vertex-wise measures, and LM starts to overestimate the R2 with more predictors. 

This suggests that LMM results comparing low-dimensional atlases reported in the primary analysis should be interpreted with caution as they are likely overestimating the overall explained variance based on the LMM assumptions of a normal distribution of effects. 

The same effect is less likely happening for atlases with higher dimensions where LMM models provide smaller R2 estimates, probably because they account for the correlation structure between predictors which LMs do not. 

Different methods seem to overestimate depending on whether we're dealing with few variables or more variables

  - LMM assumption is problematic for coarse atlases with few ROIs
  
  - LM and LASSO overestimate the effects for high-dimensional atlases probably because they don't account for population structure
