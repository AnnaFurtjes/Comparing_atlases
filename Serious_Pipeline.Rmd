---
title: "Developing atlas processing pipeline"
...
As the data is very large, we proceed to process each participant individually, so we can then save the information we need, and delete the rest to make space for more data. 

<br>
<br>

## 1. Installing FreeSurfer v6.0.0
...
## 2. Download atlas annotation files
...
## 3. Extract atlases on individual level

### 3.1 Mapping from .gcs files 
```{bash}
	####################################################################################
...
### 3.2 Mapping from .annot file (Barcelona pipeline from CJNeuroLab)
...
## 5. Problems with Schaefer atlas
...
#SBATCH --array=1-20%20 ### later more something like 1-40000%300 which should finish within the 7day (168h) limit
